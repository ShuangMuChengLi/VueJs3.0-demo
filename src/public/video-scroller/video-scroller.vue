...
export default {
  ...
  methods:{
    ...
    initCanvas(){
      ...
      this.$nextTick(() => {
        ...
        if (this.pendingEvent.length){
          let tempAry = [];
          let length = this.pendingEvent.length;
    ...
    drawGraduationRuler(){
      ...
      let space = Math.floor(this.canvasWidth / 96);// 刻度之间的距离
      let translateX = (this.canvasWidth - (space * 96)) / 2 + space;
    ...
    checkScrollerCrossBorder(source){
      let maxScrollerBorder = this[source].maxScrollerWidth - this[source].width;
    ...
    checkScrollerWidthCrossBorder(source){
      let maxWidth = this[source].maxScrollerWidth;
      let minWidth = 100;// 手动设置最小宽度为100

      let viewWidth = this[source].width + this[source].nowPosition;// 滑块宽度加上left
    ...
    calculateResult(){
      ...
      let startSends = moment(_.get(this.timeData, 'videoDate', '') + ' ' + _.get(this.timeData, 'videoStartTime')).valueOf();
      let endSeconds = moment(_.get(this.timeData, 'videoDate', '') + ' ' + _.get(this.timeData, 'videoEndTime')).valueOf();
      let timeRange = endSeconds - startSends; // 时间差值（毫秒）

      // 算出底下滚动条每一个像素点时间对应长度
      let pixelTimeScroller = timeRange / this.scrollerInfo.maxScrollerWidth;
      let scrollerStartTime = pixelTimeScroller * this.scrollerInfo.nowPosition;

      // 刻度尺的时间范围
      let rulerTimeRange = pixelTimeScroller * this.scrollerInfo.width;
      // 刻度尺每个像素对应的时间长度
      let pixelRuler = rulerTimeRange / this.rulerInfo.maxScrollerWidth;
      let rulerStartTime = this.rulerInfo.nowPosition * pixelRuler;
      let rulerEndTime = (this.rulerInfo.nowPosition + this.rulerInfo.width) * pixelRuler;

      // 根据计算得出正确的时间是
      let resultStartTime = Math.ceil(scrollerStartTime + rulerStartTime + startSends);
      let resultEndTime = Math.ceil(rulerEndTime + startSends + scrollerStartTime);

      let startTime = moment(resultStartTime).format('YYYY-MM-DD HH:mm:ss');
      let endTime = moment(resultEndTime).format('YYYY-MM-DD HH:mm:ss');
